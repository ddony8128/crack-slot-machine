...
|----|-------|-------|------|--------|
| 1 | Config structures | `lib/symbols/sets.ts`, `lib/rules/sets.ts`, `lib/modes/config.ts` + types + integrity tests. 6 symbol sets (number/fruit/gem/cat/vehicle/monster), rule-set configs (daily_basic_1/2, spire_basic_temp). No engine wiring. | low | ☐ |
| 2 | Score tweaks | 4 penalty 20→30; confirm numbers excluded from poker hands; tests. | low | ☐ |
| 3 | Seed start board | `lib/board/initialBoard.ts` — `initialBoardFor(seed, weights)` (deterministic) + tests. | low | ☐ |
| 4 | Engine event log | `EngineEvent` type; cascade emits rerolled/transformed/moved/copied/locked; attach to SpinLog; tests. | med | ☐ |
| 5 | Config-driven scoring | `score.ts` generalized: hands over all non-number symbols in active sets; per-set bonuses (fruit/gem 3종·올, cat neighbor, vehicle move/reroll via events, monster copy via events). | high | ☐ |
| 6 | New symbols in engine | extend `SymbolType` + `BASE_WEIGHTS` + `SymbolView` for cat/vehicle/monster; tests. | med | ☐ |
| 7 | Daily ad-refill | migration `0003`; `daily_user_status`; 5 base +5 ad → 10 max; dummy-ad modal + refill API; attempts UI; tests. | med | ☐ |
